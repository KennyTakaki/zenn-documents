...
## huskyがどのようにhooksを実行できるかの仕様理解
Husky を導入すると、`pre-commit` や `pre-push` などの Git フックを簡単に設定できるようになります。この仕組みを理解したかったので、少し調べて記録に残します。(huskyのバージョンは9系です。どうやら8系との間で仕様が少し変わっているようです。)
